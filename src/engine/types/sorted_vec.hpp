...
namespace eng {
    ...
    template <typename INSERT_BHV, typename DATA_TYPE, typename COMP_LESS = std::less<>>
    class SortedVector_TMPL {
      ...
      public:
        ...
        constexpr DATA_TYPE *try_find(VAL &&v, PRED p = COMP_LESS{}) {
            auto it = std::equal_range(
                _data.begin(), _data.end(), v, [&p](auto &&e, auto &&v) { return p(e, v); });

            if (it.first != it.second) { return &*it.first; }
            return nullptr;
        }
        template <typename VAL, typename PRED = COMP_LESS>
        constexpr DATA_TYPE *try_find(VAL &&v, PRED p = COMP_LESS{}) const {
            auto it = std::equal_range(
                _data.begin(), _data.end(), v, [&p](auto &&e, auto &&v) { return p(e, v); });
        ...
        template <typename VAL, typename PRED = COMP_LESS>
        constexpr bool contains(VAL &&v, PRED p = COMP_LESS{}) {

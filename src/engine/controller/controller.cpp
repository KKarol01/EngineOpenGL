...
static void glfw_callback_handler(GLFWwindow *w, int k, int s, int a, int m) {
    Engine::instance().controller()->_key_callback(w, k, s, a, m);
}

Controller::Controller() { glfwSetKeyCallback(Engine::instance().window()->glfwptr(), (GLFWkeyfun)glfw_callback_handler); }

...
void ray_box(const in Ray r, const in vec3 bmin, const in vec3 bmax, out RayHitInfo info) {
    vec3 invdir = 1. / r.d;
...
void main() {
    ...
    for (int i = 0; i < samples; ++i) {
        ...
        float yatt = 0.7;
        float xatt = snoise(nc * (snoise(tc * 4.4 + .5 * time * vec3(3.0, -7., 0.) * 0.9 + 1.3))
                            + .5 * time * vec3(.0, -1., 0.))
                     + 2. * (1. - (tc.y * .5 + .5));
        n *= xatt * yatt;
        n = pow(n, 1.);
        ...
        float lsc     = length(sc);
        float sdnoise = snoise(sc * 1.3 + .5 * time * vec3(0., -1., 0.)) * xatt * yatt;
    ...
    acc /= samples * 2.3;
    acc = 3.5 * pow(acc, vec3(1., 2.5, 4.5));

    FRAG_COL   = vec4((acc), pow(acc.x, 0.5));
